...
const Intr DEFAULT_CAM_PARAMS   =   {
                        ...
                        640,            //320,           //640,           
                        480,            //240,           //480,
                        618.958f,            //309.479f,           //535.4f,
                        618.969f,            //309.485f,          //539.2f,
                        327.26f,            //163.63f,           //320.1f,
                        237.817,            //118.908f,           //247.6f,
                                                ( 1.f / 5000.f )
                                };
...
load_names_of_all_files_from_dir ( const char * dir_name, vector < string > & vector_with_names )
{
    ...
    stringstream dir(dir_name);
    string seg_name;
    vector <string> seg;
    while(getline(dir,seg_name,'/')){
        seg.push_back(seg_name);
    }
    cout<<"load "<<*(seg.end()-1)<<" file(s):"<<endl;
    int i = 0;
    ...
    sort(vector_with_names.begin(),vector_with_names.end());
    for (vector<string>::iterator iter = vector_with_names.begin();iter < vector_with_names.end();++iter)
        cout<< *iter << endl;
    cout<<"finish loading "<<i<<" file(s)"<<endl<<endl;
}

void
point_cloud_file_name(const char * dir_name, vector < string > & vector_with_names, size_t number)
{
    string s(dir_name);
    string name("/point_cloud");
    string extension (".pcd");
    cout<<"output " << number << " point clouds:"<<endl;
    for(int i=0;i<number;++i)
    {
        vector_with_names.push_back(s+name+boost::to_string(i+1)+extension);
    }
}
...
main ( int argc, char* argv[] )
{
    ...
    if ( argc == 4 )
    {
        ...
        point_cloud_file_name ( argv[3], pcd_file_names, depth_names.size());
    }
    ...
    cout<<"finish writing point clould file(s) to designated directory"<<endl;
    return 0;
}